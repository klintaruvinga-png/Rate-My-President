...
- [x] **UI #5** Add Daily Prompt row to `SwipeCard` (rotating microcopy per DESIGN.md:223). **PR #43 committed, pending build+tsc gate.**
- [x] **UI #7** Implement Streak Counter component (amber accent, Streak icon, `aria-live`). **PR #43 committed, pending build+tsc gate.** (client-side localStorage streak per anonymous UUID; no backend change)
